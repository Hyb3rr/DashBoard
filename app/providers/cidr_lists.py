...
def refresh_cidr_source(conn, source: str, url: str, kind: str, cache: Path | None = None) -> dict:
    ...
    for network in networks:
        conn.execute("""INSERT INTO privacy_networks(network,kind,source,first_seen,last_seen,checked_at,metadata_json,active)
          ...
          (network, kind, source, now, now, now, "{}"))
        conn.execute("INSERT INTO privacy_network_history(network,kind,source,first_seen,last_seen,observed_at,metadata_json) VALUES(?,?,?,?,?,?,?)",
                     (network, kind, source, now, now, now, "{}"))
    conn.commit()
