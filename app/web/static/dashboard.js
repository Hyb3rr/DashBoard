...
function trafficTime(value){return window.formatVnTime(value)}
...
function renderTraffic(data){
  ...
  $('traffic-summary').textContent=`${num(data.total_requests||0)} requests · ${esc(data.range_label||'selected range')} · updated ${trafficClock(data.as_of)}`;
  renderTrafficFilter(data);
  {
    ...
    const riskSeries=(data.risk_series||[]).filter(x=>x.timestamp),riskAt=t=>riskSeries.find(x=>Math.abs(new Date(x.timestamp).getTime()-new Date(t).getTime())<1000)||{};
...
function showRiskTransitions(transitions){
  ...
  const bad=fresh.filter(x=>x.new_label==='critical').length;
  if(fresh.length===1){
    const item=fresh[0];
    const type=item.new_label==='critical'?'bad':'warn';
    notify(`🚨 IP ${item.ip} elevated to ${classificationLabel(item.new_label).toUpperCase()}`,type,6000,item.ip);
  }else{
    const targetIp=fresh.find(x=>x.new_label==='critical')?.ip||fresh[0].ip;
    notify(`${fresh.length} new risk changes · ${bad} CRITICAL · ${fresh.length-bad} MEDIUM`,bad?'bad':'warn',6000,targetIp);
  }
}
...
function applyView(){
  ...
  $('page-kicker').textContent=meta.kicker;$('page-title').textContent=meta.title;const subtitle=$('page-subtitle');if(subtitle)subtitle.textContent=meta.subtitle||'';
}
...
function renderHealthDialog(data){
  ...
  $('health-last-checked').textContent='Last checked: '+window.formatVnTime(new Date());
